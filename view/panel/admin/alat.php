...
// tambah data
    if(isset($_POST['btn-alat'])){
        ...
        $harga      = str_replace(",", "", htmlspecialchars($_POST['mharga']));
        $jumlah     = htmlspecialchars($_POST['mjumlah']);
        $total      = str_replace(",", "", htmlspecialchars($_POST['mtotal']));

        $field = 'id_alat,nama_alat,id_supplier,id_proyek,tanggal,harga,jumlah,total';
        $data = "null,'$nm','$supId','$proyekId','$tanggal','$harga','$jumlah','$total'";
        if(tambahData("alat_bantu", $field, $data) > 0 ){
            echo "<script>
...
// tampil data 
    $alat = tampilData("alat_bantu");

// hapus data 
    if(isset($_POST['hapus']) > 0){

       if( hapus("alat_bantu","id_alat",$_POST) ){
           echo "<script>
                    alert('Data Berhasil di Hapus.');
                    document.location.href='{$_SESSION['baseAdmin']}?panel=alat';
                  </script>
            ";
       }
    }
// ubah data
if(isset($_POST['altubah'])){
    ...
    if(ubahdata( $data, "alat_bantu", $id ) > 0 ){
            echo "<script>
                                ...
                                <form method="post" onsubmit="" class="form-horizontal kontraktor">
                                    <div class="modal-content" >
                                        <div class="modal-header">
                                            <h4 class="modal-title jdlForm">Tambah Alat</h4>
                                        </div>
                                        <div class="modal-header" style="">
                                            <div class="">
                                                <label for="nm" class="form-field-8">Nama Alat</label>
                                           ...
                                           <!-- tambah -->
                                            ...
                                            <div class="mt-1 supIdu">
                                                ...
                                                <select name="supIdu" id="supidu" class="form-control" required="">
                                                    ...
                                                    <?php 
                                                        $suplayer = tampilIfExist("supplier","data_alat","id_supplier");
                                                        foreach ($suplayer as $sup) :?>
                                                           <option value="<?= $sup['id_supplier'] ?>">
                                                                <?= $sup['nama'] ?> Kategori <?= $sup['jenis'] ?>
                                                            </option> 
                                            ...
                                            <div class="mt-1 proyekIdu">
                                                <label for="nm" class="form-field-8">Proyek</label>
                                                    <select name="proyekIdu" id="proid" class="form-control" required="">
                                                        ...
                                                        <?php 
                                                            $Proyek = tampilIfExist("data_proyek","data_alat","id_proyek");
                                                            foreach ($Proyek as $pro) :?>
                                                            <option value="<?= $pro['id_proyek'] ?>">
                                                                    <?= $pro['nama_proyek'] ?> 
                                                                </option> 
                                        ...
                                        <div class="modal-footer">
                                            <button type="reset" class="btn batalalt btn-white btn-warning btn-bold" data-dismiss="modal"><i class="ace-icon fa fa-exclamation-circle bigger-110  "></i> <span>Reset</span></button>
                                            <button type="submit" name="btn-alat" class="btn btn-white btn-primary btn-bold savealt"><i class="ace-icon fa fa-check-square-o bigger-110 blue"></i><span>Simpan</span></button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                            <div class="col">
                                ...
                                <div class=" table-responsive">
                                    ...
                                    <tbody>
                                        ...
                                        $no = 1;
                                            foreach ($alat as $alt) :
                                            $proyek = tampilByID("data_proyek","id_proyek",$alt['id_proyek']);
                                            $suplayer = tampilByID("supplier","id_supplier",$alt['id_supplier']);
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><?= $suplayer['nama'] ?></td>
                                                <td class="money"><?= $alt['harga'] ?></td>
                                                <td class=""><?= $alt['jumlah'] ?></td>
                                                <td class="money"><?= $alt['total'] ?></td>
                                                <td style="text-align:center" class="hilang">
                                                    <div class="hidden-sm hidden-xs action-buttons">
                                                        <a class="green altubah" href="#"
                                                        data-id = "<?= $alt['id_alat'] ?>"
                                                        data-nmalat = "<?= $alt['nama_alat'] ?>"
                                                        data-nmproyek = "<?= $proyek['nama_proyek'] ?>"
                                                        data-idproyek = "<?= $alt['id_proyek'] ?>"
                                                        data-nmsuplayer = "<?= $suplayer['nama'] ?>"
                                                        data-idsuplayer = "<?= $alt['id_supplier'] ?>"
                                                        data-harga = "<?= $alt['harga'] ?>"
                                                        data-jumlah = "<?= $alt['jumlah'] ?>"
                                                        data-total = "<?= $alt['total'] ?>"
                                                        >
                                                            <i class="ace-icon fa fa-pencil bigger-130"></i>
                                                        </a>

                                                        <a class="red althps" href="#" data-toggle = "modal"
                                                        data-target = "#althps"
                                                        data-id     = "<?= $alt['id_alat'] ?>"
                            ...
                            <!-- MODAL -->
                                 <!-- modal hps -->
                                    <form method="post" onsubmit="">
                                        <div id="althps" class="modal fade">
